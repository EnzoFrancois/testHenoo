...
<div class="container" id="tuto">
  <TitleAndDate/>
      ...
      <div class="panel panel-primary" v-show="listCars.length">
        <div class="panel-heading"><h1>Tableau des voitures</h1></div>        
        <table class="table table-bordered table-striped table-responsive-md">
          <thead >
            <tr class="table-dark">
             <th class="col-2">Marque voiture</th>
          ...
          <tbody class="corps">
            <tr v-for="(car, index) in listCars" :key="index">
              <td>{{ car.nom_modele }}</td>
              <td><ul class="list-unstyled">
                <li> {{car.couleur.name}} </li>
                <li><button class="apercu" :style="{backgroundColor: car.couleur.code}">
                  <strong>{{car.couleur.code}}</strong></button></li>
                </ul>
              </td> 
              <td><ul class="list-unstyled">
                <li v-for="(tag,id) in car.tags" :key="id">
                  <b-button variant="dark" class="badges">#{{tag}}</b-button>
                </li>
                </ul>
              </td>
              <td> <img class="table-image" :src="car.image_voiture" style="width:340px; height:200px;"/></td>
              <td><button class="btn btn-danger btn-block mt-5" @click="supprimer(index)">Supprimer</button>
                  <button class="btn btn-info btn-block mt-5" @click="copier(car)">Copier</button>
              </td>  
            </tr>  
          </tbody>       
        </table>
